...
export const createSettlement = mutation({
    ...
    handler:async (ctx, args) => {
        ...
        if (args.paidByUserId === args.receivedByUserId) {
            throw new Error("Payer an receiver cannot be the same user");
        }

        if (caller._id !== args.paidByUserId && caller.id !== args.receivedByUserId) {
    ...
    handler: async (ctx, args) => {
        ...
        if (args.entityType === "user") {
            ...
            for (const exp of expenses) {
                ...
                if (!involveMe || !involveThem) continue;
