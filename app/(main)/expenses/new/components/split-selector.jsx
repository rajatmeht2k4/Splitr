...
const SplitSelector = ({
  ...
  useEffect(() => {
    ...
    if (type === "equal") {
      ...
      // Intializse percentage splits evenly
      ...
      // Intializse exact splits evenly
  ...
  const updatePercentageSplit = (userId, newPercentage) => {
    // Update just htis user's percentage and recalculate amount
      ...
      {splits.map((split) => (
        ...
        >
          <div className='flex items-center gap-2 min-w-[120px]'>
            ...
            <span className='text-sm'>{split.userId === user?.id ? "You" : split.name}</span>
          </div>
